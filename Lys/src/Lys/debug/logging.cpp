...
namespace Lys
{
	...
	Logger::Logger() :
		_logFile(nullptr), _showLogs(true), _writeLogs(false), _showTimestamp(true),
		_minLogLevel(LogLevel::DEBUG), _timeFormat(new std::string("%H:%M:%S")),
		_configFile(nullptr)
	{
		_singleton = this;
	}
	...
	{
		_logFile = std::make_unique<std::string>(file);
	}
